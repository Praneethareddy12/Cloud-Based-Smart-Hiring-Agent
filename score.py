...
def evaluate_resume_pipeline(pdf_path, write_csv: bool = True):
...
def main(pdf_path):
    score, resume_data, github_data = evaluate_resume_pipeline(pdf_path, write_csv=True)

    if not score:
        print(f"Error: Failed to evaluate resume {pdf_path}")
        return score
