...
class PDFHandler:
    ...
    def extract_json_from_text(self, resume_text: str) -> Optional[JSONResume]:
        try:
            full_resume = self._extract_full_resume(resume_text)
            if full_resume:
                return full_resume
            logger.info(
                "🔄 Falling back to section-by-section extraction after full resume parsing failed."
            )
    ...
    def _extract_full_resume(self, resume_text: str) -> Optional[JSONResume]:
        prompt = self.template_manager.render_template(
            "full_resume", text_content=resume_text
        )
        if not prompt:
            logger.error("❌ Failed to render full resume extraction template")
            return None
            ...
            "messages": [
                {
                    ...
                    "content": self.template_manager.render_template(
                        "system_message", section_name_param="full resume"
                    ),
                },
                {"role": "user", "content": prompt},
            ],
        ...
        kwargs = {"format": JSONResume.model_json_schema()}
        ...
        try:
            ...
            if not isinstance(transformed_data, dict):
                logger.error("❌ Full resume extraction returned unexpected data type")
                return None

            json_resume = JSONResume(**transformed_data)
            logger.info("✅ Full resume extraction succeeded")
            return json_resume
        except Exception as e:
            logger.error(f"❌ Full resume extraction failed: {e}")
            return None
